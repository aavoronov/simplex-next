...
import { actionExit } from "../../store/actions/modal";
import { resetUser } from "@/store/userSlice";
import { setCookie } from "cookies-next";
import { useRouter } from "next/router";

export default function Exit() {
  const exitModal = useAppSelector((state) => state.modalExit);
  const dispatch = useAppDispatch();
  const exitModalAction = () => dispatch(actionExit());
  ...
  return (
    <>
      <button onClick={exitModalAction} className='btn btn_profile d-flex align-items-center justify-content-between w-100'>
        <div className='btn_profile-name'>Выйти</div>
      </button>
      <Modal
        isOpen={exitModal}
        onRequestClose={exitModalAction}
        contentLabel=''
        className='allModal allModal_min d-flex flex-column align-items-center'>
        <button onClick={exitModalAction} className='btn btn_modal-close position-absolute'>
        ...
        <div className='modal-title text-center'>Выйти из профиля?</div>
        <div className='modal-content d-flex flex-column align-items-center w-100'>
          <div className='modal-exit-text w-100 mx-auto text-center'>Для авторизации нужно будет произвести вход повторно</div>
        </div>
        <div className='modal-exit_buttons d-flex align-items-center justify-content-between w-100'>
          <button onClick={exitModalAction} className='btn btn_cancel'>
            Отменить
          </button>
          <button className='btn_logout d-flex align-items-center justify-content-center' onClick={logout}>
            Выйти
          </button>
        </div>
      </Modal>
