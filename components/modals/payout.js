...
import { actionPayout } from "../../store/actions/modal";
...
export default function Payout() {
  const payoutModal = useAppSelector((state) => state.modalPayout);
  const dispatch = useAppDispatch();
  const payoutModalAction = () => dispatch(actionPayout());

  return (
    <>
      <Modal
        isOpen={payoutModal}
        onRequestClose={payoutModalAction}
        contentLabel=''
        className='allModal modal-pay d-flex flex-column align-items-center'>
        <button onClick={payoutModalAction} className='btn btn_modal-close position-absolute'>
          ...
          {(props) => (
            <Form className='filter-form d-flex flex-column align-items-center w-100'>
              ...
              <div className='filter-footer d-flex justify-content-between'>
                <div className='modal-pay-info w-100'>Выплата в течение 24 часов. Можно отменить</div>
                <div className='modal-pay-price d-flex flex-column justify-content-between'>
                  <div className='modal-pay-price-value'>825 ₽</div>
                  <div className='modal-pay-price-text'>Выплата</div>
                </div>
