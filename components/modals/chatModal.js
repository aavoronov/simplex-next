...
import { actionChat } from "../../store/actions/modal";

export default function ChatModal() {
  const chatModal = useAppSelector((state) => state.modalChat);
  const dispatch = useAppDispatch();
  const chatModalAction = () => dispatch(actionChat());

  return (
    <>
      <Modal
        isOpen={chatModal}
        onRequestClose={chatModalAction}
        contentLabel=''
        className='allModal allModal_min allModal_chat d-flex flex-column align-items-center'>
        <button onClick={chatModalAction} className='btn btn_modal-close position-absolute'>
        ...
        <div className='modal-chat-icon'>
          <img src='/images/simplex.svg' alt='' />
        </div>
        <div className='modal-chat-title text-center'>
          Чат только на <span>Sim</span>pleX
        </div>
        <div className='modal-chat-text w-100 mx-auto text-center'>
          В целях безопасности не переводите деньги и не общайтесь за пределами сайта
        </div>
        <button onClick={chatModalAction} className='btn btn_submit btn_submit-green'>
          Понятно, спасибо
        </button>
      </Modal>
    </>
